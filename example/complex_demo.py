...
DUCKDB_DIR = "./tmp/duckdb_complex"       # Directory for DuckDB data
COLLECTION_NAME_PREFIX = "complex_test_collection"

DIMENSION = 8  # Dimension of the embedding vector
BATCH_SIZE = 50000 # Batch size for data operations

# Total operations: 10 million
...
def generate_data(start_id, count, for_upsert=False):
    ...
    for i in range(count):
        ...
        record = {
            ...
            "array_field": [current_id, current_id + 1, current_id + 2, random.randint(0,100)],
...
def main():
    ...
    schema.add_field("array_field", DataType.ARRAY, element_type=DataType.INT64, max_capacity=20) # Increased max_capacity
    ...
    for i in range(0, NUM_INSERTS, BATCH_SIZE):
        ...
        logging.info(f"Inserted batch {i // BATCH_SIZE + 1}/{(NUM_INSERTS + BATCH_SIZE -1) // BATCH_SIZE}, {len(batch_data)} records.")
    ...
    for i in range(0, NUM_DELETES, BATCH_SIZE):
        ids_batch = list(range(delete_start_id + i, delete_start_id + i + min(BATCH_SIZE, NUM_DELETES - i)))
        milvus_client.delete(collection_name, ids=ids_batch)
        logging.info(f"Deleted batch {i // BATCH_SIZE + 1}/{(NUM_DELETES + BATCH_SIZE -1) // BATCH_SIZE}, {len(ids_batch)} records.")
    ...
    for i in range(0, num_updates_via_upsert, BATCH_SIZE):
        batch_data = generate_data(upsert_update_start_id + i, min(BATCH_SIZE, num_updates_via_upsert - i), for_upsert=True)
        milvus_client.upsert(collection_name, batch_data)
        logging.info(f"Upserted (update) batch {i // BATCH_SIZE + 1}/{(num_updates_via_upsert + BATCH_SIZE -1) // BATCH_SIZE}, {len(batch_data)} records.")

    # Upserts that will insert new records
    for i in range(0, num_inserts_via_upsert, BATCH_SIZE):
        batch_data = generate_data(upsert_new_insert_start_id + i, min(BATCH_SIZE, num_inserts_via_upsert - i), for_upsert=True)
        milvus_client.upsert(collection_name, batch_data)
        logging.info(f"Upserted (new insert) batch {i // BATCH_SIZE + 1}/{(num_inserts_via_upsert + BATCH_SIZE -1) // BATCH_SIZE}, {len(batch_data)} records.")
    ...
    logging.info(f"Demo finished. Collection '{collection_name}' still exists. You may want to drop it manually if not needed.")
